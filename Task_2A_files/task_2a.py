    ...
    * ArUco_corners = 
    ...
    if ids is not None:
        for i in range(len(ids)):
            marker_id = int(ids[i][0])  # Get the marker's ID
            marker_corners = corners[i][0]  # Get the corner coordinates

            # Calculate the center of the marker
            center_x = (marker_corners[:, 0].sum()) / 4
            center_y = (marker_corners[:, 1].sum()) / 4

            # Calculate the angle from the vertical axis
            blue_x = (marker_corners[0][0] + marker_corners[1][0]) / 2
            blue_y = (marker_corners[0][1] + marker_corners[1][1]) / 2
            dy = center_y - blue_y
            dx = center_x - blue_x
            if(dy!=0):
                angle = math.ceil(math.degrees(math.atan(dx/dy)))
                if(dx>=0 and dy>0): angle = angle
                elif(dx>=0 and dy<0): angle = angle + 180
                elif(dx<=0 and dy>0): angle = angle
                elif(dx<0 and dy<0): angle = angle - 180
            else:
                if(dy>0): angle = 0
                else: angle = 180
            ...
            ArUco_details_dict[marker_id] = [[int(center_x), int(center_y)], angle]
