...
# Team ID:			[ GG_1380 ]
# Author List:		[ Abhi Jain ]
# Filename:			task_2c.py
# Functions:	    [`classify_event(image), event identification(arena)` ]
###################################################################################################
...
def event_identification(arena):        # NOTE: You can tweak this function in case you need to give more inputs 
    ...
    # Use edge detection (Canny) as a simple example
    edges = cv.Canny(gray_arena, 50, 150)

    # Find contours in the edges
    contours, _ = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    # Iterate through contours and extract event images
...
def classify_event(image):
    ...
    custom_classifier = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Dropout(0.2),
        nn.Linear(512, 5)  # Adjust the number of output units
    )
    model.fc = custom_classifier
    
    model.load_state_dict(torch.load('event_classifier_model.pth'))
    ...
    # image = cv.imread(image)  # Load the image using cv2 (OpenCV)
    ...
    event = class_names[min(predicted.item(),len(class_names))]  # Assuming class_names is a list of event names    
